...
EMBED_GPU="${EMBED_GPU:-off}"
LMSTUDIO_LOAD_CHAT_ROLLBACK="${LMSTUDIO_LOAD_CHAT_ROLLBACK:-0}"
...
try:
 ...
 raise SystemExit
for row in data.get("data", []):
 if row.get("id") == os.environ["MODEL_ID"]:
...
installed_model_key() {
  local model_id="$1"
  local model_path="${2:-}"
  "$LMS_BIN" ls --json | MODEL_ID="$model_id" MODEL_PATH="$model_path" /usr/bin/python3 -c 'import json,os,sys
try:
 data=json.load(sys.stdin)
except Exception:
 print("")
...
for row in data:
 ...
 if model_id in values or (model_path and model_path in values):
...
for row in data:
 values={str(row.get("modelKey","")),str(row.get("path","")),str(row.get("indexedModelIdentifier",""))}
 if any(filename and filename in value for value in values):
  print(row.get("modelKey") or row.get("path") or "")
  raise SystemExit
' 2>/dev/null || true
}
...
if truthy "$LMSTUDIO_LOAD_CHAT_ROLLBACK"; then
...
elif [[ "$(loaded_model_state "$CHAT_ID")" == "loaded" ]]; then
  "$LMS_BIN" unload "$CHAT_ID" >/dev/null 2>&1 || "$LMS_BIN" unload --all >/dev/null 2>&1 || true
fi

if [[ -z "$(installed_model_key "$EMBED_ID" "${EMBED_MODEL_PATH:-}")" ]]; then
  "$LMS_BIN" get "$EMBED_ID" --gguf --yes
fi

if [[ "$(loaded_model_state "$EMBED_ID")" != "loaded" ]]; then
